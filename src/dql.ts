...
export const dqlLanguage = LRLanguage.define({
  name: "dql",
      ...
      indentNodeProp.add({
        ...
        Script: (context) => {
          if (context.pos + /\s*/.exec(context.textAfter)![0].length >= context.node.to) {
            let endBody: SyntaxNode | null = null;
...
export function dql() {
  return new LanguageSupport(dqlLanguage, [
    dqlLanguage.data.of({ autocomplete: localCompletionSource }),
    dqlLanguage.data.of({ autocomplete: globalCompletion }),
  ]);
}
